...
public class SocialLoginService {
   ...
   * @throws Exception
   */
  public OpenApiUserVO verifySocialLoginAccessToken(LoginTypeCodeEnum socialTypeCode, String accessToken, String userId) throws Exception {
    OpenApiUserVO result = null;
    
    try {
      if(socialTypeCode == LoginTypeCodeEnum.API_FACEBOOK_CODE) {
        // 페이스북
        result = verifyFacebookAccessToken(accessToken, userId);
      } else if(socialTypeCode == LoginTypeCodeEnum.API_KAKAO_CODE) {
        // 카카오
        result = verifyKakaoAccessToken(accessToken);
      } else if(socialTypeCode == LoginTypeCodeEnum.API_NAVER_CODE) {
        // 네이버
      } else if(socialTypeCode == LoginTypeCodeEnum.API_GOOGLE_CODE) {
