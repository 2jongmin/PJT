...
public class ComeitAuthenticationProvider implements AuthenticationProvider {
  ...
  public Authentication authenticate(Authentication authentication) throws ComeitBadCredentialsException, AuthenticationException {
    ...
    //ApplicationCodeEnum loginTypeCode = ApplicationCodeEnum.valueOf(loginType);
    LoginTypeCodeEnum loginTypeCode = LoginTypeCodeEnum.getEnumByCode(loginType);
    
    try {
      switch(loginTypeCode) {
      case API_COMEIT_CODE:
        // 일반 회원 로그인
...
//      if(ApplicationCodeEnum.API_COMEIT_CODE == loginTypeCode) {
//        // 일반 회원 로그인
//        authentication = login(request);
//      } else if(ApplicationCodeEnum.API_KAKAO_CODE == loginTypeCode) {
//        
//      } else if(ApplicationCodeEnum.API_FACEBOOK_CODE == loginTypeCode) {
//        
//      }
//      if(ApplicationCodeEnum.API_COMEIT_CODE.getCode().equals(loginType)) {
//        // 일반 회원 로그인
//        authentication = login(request);
//      } else if(ApplicationCodeEnum.API_KAKAO_CODE.getCode().equals(loginType)) {
...
//      }
  ...
  private Authentication login(HttpServletRequest request) throws Exception {
    ...
    inMemberVo.setApiUuid(request.getParameter("apiUuid"));
    inMemberVo.setPassword(request.getParameter("password"));
    ...
    if(resultMemberVo != null) {
      ...
      AuthorityInfoVO authVo = new AuthorityInfoVO(userSeq, password, nickName, LoginTypeCodeEnum.MEMBER_GROUP_CODE.getValue());
